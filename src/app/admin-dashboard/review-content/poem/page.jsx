...
import { useGetAllPoemQuery, useDeletePoemMutation } from '@/app/slices/userSlices/userApiSlice';
import { FiEdit, FiTrash2 } from 'react-icons/fi'; // Importing icons

const DashboardHeader = () => {
  const { data: testimonies, refetch } = useGetAllPoemQuery();
  const [deleteTestimony] = useDeletePoemMutation()
                  ...
                  {testimonies.data.map((testimony) => (
                      <tr key={testimony._id} className="cursor-pointer border-b border-gray-300 hover:bg-gray-200">
                        ...
                        <td className="px-4 py-2 flex justify-start gap-4">
                          <Link href={`/admin-dashboard/review-content/poem/${testimony._id}`}>
