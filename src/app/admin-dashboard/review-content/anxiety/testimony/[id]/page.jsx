...
import { useGetSingleTestimonyQuery } from '@/app/slices/userSlices/userApiSlice';
import { useParams } from 'next/navigation';

const DashboardHeader = () => {
  const params = useParams();
  const id = params.id;
  const { data: testimony } = useGetSingleTestimonyQuery(id);
      ...
      >
        ...
        <div className="mx-auto max-w-5xl rounded-lg bg-black/80 p-6 mt-10">
          {testimony ? (
            ...
            <p className="text-white text-center">Loading testimony...</p>
          )}
        </div>
      </div>
