...
import { useGetAllContentQuery, useDeleteContentMutation } from '@/app/slices/userSlices/userApiSlice';
import { FiEdit, FiTrash2 } from 'react-icons/fi'; // Importing icons

const DashboardHeader = () => {
  const { data: testimonies, refetch } = useGetAllContentQuery();
  const [deleteTestimony] = useDeleteContentMutation()
          ...
          {testimonies && testimonies.data ? (
            <div className="overflow-x-auto">
              <table className="table-auto w-full bg-[#D9D9D9] rounded-lg shadow-lg">
                ...
                <tbody>
                  {testimonies.data.filter((cont) => cont.room === 'Anxiety').map((testimony) => (
                      <tr key={testimony._id} className="cursor-pointer border-b border-gray-300 hover:bg-gray-200">
                        ...
                        <td className="px-4 py-2 flex justify-start gap-4">
                          <Link href={`/admin-dashboard/review-content/anxiety/creative-pieces/${testimony._id}`}>
